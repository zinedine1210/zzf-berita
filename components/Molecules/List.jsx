  ...
  return (
      <div className='mb-2 lg:mb-8 flex lg:block gap-3 items-center justify-center'>
            <div className="w-1/3 lg:w-full">
              {skeleton ? <Skeleton className='h-[80px] lg:h-[150px]'/> :
                <Image src={skeleton ? "/images/default.webp" : gambar()} alt="Gambar berita" width={767} height={512} objectFit="contain" />
              }
            </div>
            <div className='w-2/3 lg:w-full'>
              <h3 className="text-sm lg:mt-5 mb-2 lg:text-xs"><span className='text-sky-500 font-main font-bold text-xs'>{skeleton ? <Skeleton inline={true} width={50} height={10}/> : stuff.category_name_0}</span> {skeleton ? <Skeleton inline={true} width={50} height={10} /> : " / " + moment(new Date(stuff._cd.epoch_time*1000)).local().format("DD MMMM YYYY")}</h3>
